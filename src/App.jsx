    ...
    <div style={S.adminWrap}>
      <div style={S.tabBar}>
        {[["calendar",t("📅 月間","📅 캘린더")],
          ...
          ["members",t("👥 社員管理","👥 직원관리")],
          ["stats",t("📊 統計","📊 통계")],["attendance",t("⏰ 出退勤","⏰ 출퇴근")]].map(([k,l])=>(
...
function AttendanceTab({ lang, t, employees }) {
  ...
  useEffect(() => {
    if (empOnly.length > 0 && !selectedEmpId) setSelectedEmpId(empOnly[0].id);
  }, [employees]);

  useEffect(() => {
    if (viewMode === "date" && selectedDate) fetchRecords();
  }, [viewMode, selectedDate]);

  useEffect(() => {
    if (viewMode === "employee" && selectedEmpId) fetchRecords();
  }, [viewMode, selectedEmpId, selectedMonth]);
  ...
  async function fetchRecords() {
    ...
    try {
      let path = "";
      if (viewMode === "date") {
        path = `/attendance_records?work_date=eq.${selectedDate}&select=*&order=clock_in.asc`;
      } else {
        const [year, month] = selectedMonth.split("-");
        const startDate = `${year}-${month}-01`;
        const endDate = new Date(year, month, 0).toISOString().slice(0, 10);
        path = `/attendance_records?employee_id=eq.${selectedEmpId}&work_date=gte.${startDate}&work_date=lte.${endDate}&select=*&order=work_date.asc`;
      }
  ...
  function downloadExcel() {
    ...
    a.href = url;
    a.download = viewMode === "date" ? `出退勤_${selectedDate}.csv` : `出退勤_${getEmpName(selectedEmpId)}_${selectedMonth}.csv`;
    a.click();
  }

  const thS = { padding: "10px 12px", textAlign: "left", fontWeight: 700, color: "#374151", borderBottom: "2px solid #e2e8f0", whiteSpace: "nowrap", fontSize: 13 };
  const tdS = { padding: "10px 12px", borderBottom: "1px solid #f1f5f9", color: "#374151", fontSize: 13 };

  return (
    <div>
      <h3 style={S.secTitle}>{t("出退勤管理","출퇴근 관리")}</h3>

      {/* 보기 모드 전환 */}
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[["date", t("📅 日付別","📅 날짜별")], ["employee", t("👤 社員別","👤 직원별")]].map(([mode, label]) => (
          <button key={mode} onClick={() => setViewMode(mode)}
            style={{ flex: 1, padding: "10px", border: "none", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 14,
              background: viewMode === mode ? "#0F6E56" : "#f1f5f9",
      ...
      {viewMode === "date" && (
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6 }}>{t("日付を選択","날짜 선택")}</label>
          <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)}
            style={{ padding: "10px 14px", border: "1.5px solid #e2e8f0", borderRadius: 10, fontSize: 14, width: "100%", boxSizing: "border-box" }}/>
        </div>
      )}
      ...
      {viewMode === "employee" && (
        <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6 }}>{t("社員を選択","직원 선택")}</label>
            <select value={selectedEmpId} onChange={e => setSelectedEmpId(e.target.value)}
              style={{ padding: "10px 14px", border: "1.5px solid #e2e8f0", borderRadius: 10, fontSize: 14, width: "100%" }}>
              {empOnly.map(e => <option key={e.id} value={e.id}>{lang === "ja" ? e.name : e.name_ko}</option>)}
            </select>
          </div>
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6 }}>{t("月を選択","월 선택")}</label>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
        <button onClick={downloadExcel}
          style={{ background: "#217346", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          ⬇ Excel {t("ダウンロード","다운로드")}
        </button>
      </div>
      ...
      {/* 결과 */}
      {loading ? (
        <div style={{ textAlign: "center", padding: 30, color: "#9ca3af" }}>{t("読み込み中...","로딩 중...")}</div>
      ) : records.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f8fafc" }}>
                {viewMode === "date" ? <th style={thS}>{t("社員名","직원명")}</th> : <th style={thS}>{t("日付","날짜")}</th>}
            ...
            <tbody>
              {records.map((r, i) => (
                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#f8fafc" }}>
                  {viewMode === "date" ? <td style={tdS}>{getEmpName(r.employee_id)}</td> : <td style={tdS}>{r.work_date}</td>}
          ...
          {viewMode === "employee" && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "#f0fdf4", borderRadius: 10, fontSize: 13 }}>
              <span style={{ fontWeight: 700, color: "#065f46" }}>
                {t("出勤日数","출근일수")}: {records.filter(r => r.clock_in).length}{t("日","일")}
              </span>
            </div>
          )}
        </div>
