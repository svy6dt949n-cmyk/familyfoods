  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        if(savedId){const acc=emps.find(e=>e.login_id===savedId);if(acc)setLoggedIn(acc);}
...
export default function App(){
  ...
  useEffect(()=>{
    async function load(){
      ...
      try{
        ...
        const savedId = localStorage.getItem(LS_LOGIN);
        if(savedId){
          const acc = emps.find(e=>e.login_id===savedId);
          if(acc) setLoggedIn(acc);
        }
      }catch(e){ showToast("DB接続エラー: "+e.message,"error"); }
      finally{ setLoading(false); }
    }
    load();
  },[]);
  ...
  function login(loginId,password){
    const acc=employees.find(a=>a.login_id===loginId&&a.password===password);
    if(!acc){ showToast(t("IDまたはパスワードが違います","ID 또는 비밀번호가 틀립니다"),"error"); return; }
    try{ localStorage.setItem(LS_LOGIN, acc.login_id); }catch(e){}
    setLoggedIn(acc);
  }
  ...
  async function addEmployee(emp){
    ...
    try{
      const res = await db.addEmployee({
        ...
        password: emp.tempPassword||"temp1234",
        must_change_password: true,
        name: emp.name,
        name_ko: emp.nameKo||emp.name,
        avatar: makeAvatar(emp.name),
        dept: emp.dept||"その他",
        role: "employee",
        remaining_paid_leave: Number(emp.remainingPaidLeave)||20,
        selected_day_off: emp.selectedDayOff||"月",
      ...
      setEmployees(p=>[...p,...res]);
      showToast(t("社員を追加しました","직원 추가 완료"));
      return true;
    }catch(e){ showToast(t("エラーが発生しました","오류 발생"),"error"); return false; }
  }
  ...
  async function updateEmployee(id, data){
    ...
    try{
      ...
      if(data.nameKo) updateData.name_ko=data.nameKo;
      if(data.login_id) updateData.login_id=data.login_id;
      if(data.dept) updateData.dept=data.dept;
      if(data.selectedDayOff) updateData.selected_day_off=data.selectedDayOff;
      if(data.remainingPaidLeave!==undefined) updateData.remaining_paid_leave=Number(data.remainingPaidLeave);
      if(data.name) updateData.avatar=makeAvatar(data.name);
      await db.updateEmployee(id, updateData);
      setEmployees(p=>p.map(e=>e.id===id?{...e,...updateData}:e));
      showToast(t("更新しました","업데이트 완료"));
      return true;
    }catch(e){ showToast(t("エラーが発生しました","오류 발생"),"error"); return false; }
  }
  ...
  async function resetPassword(id, tempPw){
    try{
      await db.updateEmployee(id,{password:tempPw,must_change_password:true});
      setEmployees(p=>p.map(e=>e.id===id?{...e,password:tempPw,must_change_password:true}:e));
      showToast(t(`仮パスワードを「${tempPw}」に設定しました`,`임시 비밀번호를 「${tempPw}」로 설정했습니다`));
    }catch(e){ showToast(t("エラーが発生しました","오류 발생"),"error"); }
  }
  ...
  async function approve(id){
    const req=requests.find(r=>r.id===id);
    const sameDay=requests.filter(r=>r.date===req.date&&r.status==="approved"&&r.type===req.type&&r.id!==id);
    if(req.type==="選択休暇"&&sameDay.length>=MAX_OFF){
      showToast(t("定員超過","정원 초과"),"error"); return;
    }
    try{
      await db.updateRequest(id,{status:"approved"});
      setRequests(p=>p.map(r=>r.id===id?{...r,status:"approved"}:r));
      if(req.type==="有給休暇"){
        const dec=req.half?0.5:1;
        const emp=employees.find(e=>e.id===req.emp_id);
        const newLeave=Math.max(0,(emp?.remaining_paid_leave||0)-dec);
        await db.updateEmployee(req.emp_id,{remaining_paid_leave:newLeave});
        setEmployees(p=>p.map(e=>e.id===req.emp_id?{...e,remaining_paid_leave:newLeave}:e));
  ...
  function getWarnings(){
    const w=[];
    for(let d=1;d<=getDays(year,month);d++){
      const ds=fmt(year,month,d); if(isOff(ds)) continue;
      const absent=requests.filter(r=>r.date===ds&&r.status==="approved").length;
      const total=employees.filter(e=>e.role==="employee").length;
      if(total>0&&absent>=total-2) w.push(ds);
    }
    return w;
  }
  ...
  const dayLabels=lang==="ja"?DAY_LABELS_JP:DAY_LABELS_KO;
  const getApproved=(ds)=>requests.filter(r=>r.date===ds&&r.status==="approved");
  const getPending=(ds)=>requests.filter(r=>r.date===ds&&r.status==="pending");
  const getAllForDate=(ds)=>requests.filter(r=>r.date===ds);
  const getMyReq=(empId,ds)=>requests.find(r=>r.emp_id===empId&&r.date===ds);
  const currentEmp=loggedIn?.role==="employee"?(employees.find(e=>e.id===loggedIn.id)||loggedIn):loggedIn;

  const shared={lang,t,employees,requests,showToast,
    submitRequest,approve,reject,cancel,downloadCSV,
    warn,pendingList,year,month,prevMonth,nextMonth,
    daysInMonth:getDays(year,month),firstDay:getFirst(year,month),
    dayLabels,getApproved,getPending,getAllForDate,getMyReq};
  ...
  if(loading) return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#6366f1,#8b5cf6)"}}>
      ...
      <div style={{color:"rgba(255,255,255,0.7)",fontSize:13,marginTop:8}}>読み込み中…</div>
      <div style={{width:40,height:40,border:"4px solid rgba(255,255,255,0.3)",borderTop:"4px solid #fff",borderRadius:"50%",animation:"spin 1s linear infinite",marginTop:20}}/>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </div>
  );
  ...
  return (
    <div style={S.root}>
      ...
      {!loggedIn&&<LoginScreen lang={lang} setLang={setLang} t={t} onLogin={login}/>}
      {loggedIn&&currentEmp?.must_change_password&&(
        <ForceChangePw lang={lang} t={t} user={currentEmp} onSave={(pw)=>changePassword(currentEmp.id,pw)} onLogout={logout}/>
      )}
      {loggedIn&&!currentEmp?.must_change_password&&(
        <>
          <Header lang={lang} setLang={setLang} t={t} user={currentEmp} onLogout={logout}/>
    ...
    <div style={S.loginBg}>
      <div style={S.loginCard}>
        ...
        <div style={S.fg}>
          <label style={S.fl}>{t("ログインID","로그인 ID")}</label>
          <input style={S.input} value={id} onChange={e=>setId(e.target.value)}
            placeholder={t("IDを入力してください","ID를 입력하세요")} autoComplete="username"/>
        </div>
        <div style={S.fg}>
          <label style={S.fl}>{t("パスワード","비밀번호")}</label>
          <div style={{position:"relative"}}>
  ...
  return (
    <div style={S.loginBg}>
      <div style={S.loginCard}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{fontSize:40,marginBottom:8}}>🔑</div>
          <div style={S.loginTitle}>{t("パスワードの変更","비밀번호 변경")}</div>
          <div style={{fontSize:13,color:"#6b7280",marginTop:6}}>
            {t("初回ログインです。新しいパスワードを設定してください。","첫 로그인입니다. 새 비밀번호를 설정해주세요.")}
          </div>
        </div>
        <div style={{background:"#f8fafc",borderRadius:10,padding:"12px 16px",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
          <div style={S.smAv}>{user.avatar}</div>
          <div>
            <div style={{fontWeight:700,fontSize:14}}>{lang==="ja"?user.name:user.name_ko}</div>
            <div style={{fontSize:12,color:"#9ca3af"}}>ID: {user.login_id}</div>
          </div>
        </div>
        <div style={S.fg}>
          <label style={S.fl}>{t("新しいパスワード","새 비밀번호")}</label>
          <div style={{position:"relative"}}>
            <input style={{...S.input,paddingRight:40}} value={pw} type={show?"text":"password"}
              onChange={e=>{setPw(e.target.value);setErr("");}}/>
            <button style={S.eyeBtn} onClick={()=>setShow(v=>!v)}>{show?"🙈":"👁"}</button>
        ...
        <div style={S.fg}>
          <label style={S.fl}>{t("パスワード確認","비밀번호 확인")}</label>
          <input style={S.input} value={pw2} type="password"
  ...
  return (
    <header style={S.header}>
      <div style={S.logo}>
        <div style={S.logoIcon}><SmileLogo size={28}/></div>
        <div>
          <FamilyFoodsText size={14}/>
          <div style={S.logoSub}>{t("勤怠管理システム","근태관리 시스템")}</div>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <LangToggle lang={lang} setLang={setLang}/>
    ...
    <div style={S.empLayout}>
      <aside style={S.sidebar}>
        <div style={S.card}>
          ...
          <button style={{...S.editBtn,width:"100%",marginTop:12,textAlign:"center"}} onClick={()=>setShowChgPw(true)}>
            🔑 {t("パスワード変更","비밀번호 변경")}
          </button>
        </div>
        <div style={S.card}>
          <div style={S.statTitle}>{t("有給残日数","잔여 유급")}</div>
          <div style={S.statBig}>{currentUser.remaining_paid_leave}<span style={{fontSize:14,color:"#9ca3af"}}>{t("日","일")}</span></div>
          <div style={S.bar}><div style={{...S.barFill,width:`${(currentUser.remaining_paid_leave/20)*100}%`}}/></div>
          <div style={{fontSize:11,color:"#9ca3af"}}>{t("取得済","사용")}: {usedPaid}{t("日","일")}</div>
        </div>
        <div style={S.card}>
          <div style={{...S.statTitle,marginBottom:10}}>{t("申請履歴","신청 이력")}</div>
          ...
          :myReqs.map(r=>(
            <div key={r.id} style={S.histItem}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:600,fontSize:12}}>{r.date}</span>
                <span style={{...S.pill,background:SC[r.status]}}>{SL[r.status]}</span>
              </div>
              <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>{r.type}{r.half?t("（半休）","（반차）"):""}</div>
              {r.status==="pending"&&<button style={S.cancelBtn} onClick={()=>cancel(r.id)}>{t("キャンセル","취소")}</button>}
            </div>
          ))}
        </div>
        <button style={{...S.primaryBtn,width:"100%"}} onClick={()=>setModal({date:null})}>
      ...
      <div style={S.calWrap}>
        ...
        <CalGrid year={year} month={month} daysInMonth={daysInMonth} firstDay={firstDay} dayLabels={dayLabels}
          renderDay={(ds,dn)=>{
            const my=getMyReq(currentUser.id,ds);
            const appr=getApproved(ds); const w=warn.includes(ds);
            return (
              <div key={ds} style={{...S.dayCell,...(isOff(ds)?S.dayCellOff:{}),
                ...(w&&!isOff(ds)?{border:"1.5px solid #f59e0b",background:"#fff7ed"}:{})}}
                onClick={()=>!isOff(ds)&&setModal({date:ds})}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <span style={{...S.dayNum,...dayColor(ds)}}>{dn}</span>
                  {isHoliday(ds)&&<span style={S.holTag}>{JP_HOLIDAYS[ds]?.slice(0,3)}</span>}
                  {w&&!isOff(ds)&&<span style={{fontSize:11}}>⚠</span>}
                </div>
                {my&&<div style={{...S.chip,background:my.status==="approved"?"#d1fae5":my.status==="pending"?"#fef3c7":"#fee2e2",
                  color:my.status==="approved"?"#065f46":my.status==="pending"?"#92400e":"#991b1b"}}>
                  {my.type==="有給休暇"?(my.half?t("半休","반차"):t("有給","유급")):t("選休","선택")}
                </div>}
                {!my&&!isOff(ds)&&appr.length>0&&<div style={{...S.chip,background:"#e0f2fe",color:"#0369a1"}}>{appr.length}名</div>}
              </div>
            );
          }}
        />
        <Legend items={[{bg:"#d1fae5",tc:"#065f46",label:t("承認済","승인")},{bg:"#fef3c7",tc:"#92400e",label:t("審査中","심사중")},
          {bg:"#fee2e2",tc:"#991b1b",label:t("却下","반려")},{bg:"#fff7ed",tc:"#92400e",label:t("⚠ 人員不足","⚠ 인원부족")}]}/>
      </div>
      {modal&&<ReqModal t={t} initDate={modal.date} empId={currentUser.id}
        onSubmit={f=>{submitRequest(f);setModal(null);}} onClose={()=>setModal(null)}/>}
      {showChgPw&&<ChangePwModal t={t} user={currentUser} onSave={onChangePassword} onClose={()=>setShowChgPw(false)}/>}
    </div>
  );
}
...
// ══════════════════════════════════════════════════════════════════
function AdminView({lang,t,employees,allAccounts,requests,year,month,prevMonth,nextMonth,
  daysInMonth,firstDay,dayLabels,warn,getApproved,getPending,getAllForDate,
  approve,reject,downloadCSV,pendingList,
  addEmployee,updateEmployee,deleteEmployee,resetPassword}){
  const [tab,setTab]=useState("calendar");
  const [filter,setFilter]=useState("all");
  const [dayModal,setDayModal]=useState(null);
  const SC={approved:"#10b981",pending:"#f59e0b",rejected:"#ef4444"};
    ...
    <div style={S.adminWrap}>
      ...
      <div style={{padding:"16px 20px"}}>
        {tab==="calendar"&&(<>
          ...
          {warn.length>0&&(<div style={S.warnBanner}>
            ⚠ {t(`${warn.length}日間、人員不足の恐れ: `,`${warn.length}일 인원부족: `)}
            {warn.map(w=><span key={w} style={S.warnDate}>{w.slice(5)}</span>)}
          </div>)}
          <CalGrid year={year} month={month} daysInMonth={daysInMonth} firstDay={firstDay} dayLabels={dayLabels}
            renderDay={(ds,dn)=>{
              const all=getAllForDate(ds).filter(r=>filter==="all"||r.emp_id===+filter);
              const appr=all.filter(r=>r.status==="approved"); const pend=all.filter(r=>r.status==="pending"); const w=warn.includes(ds);
              return (
                <div key={ds} style={{...S.dayCell,...(isOff(ds)?S.dayCellOff:{}),
                  ...(w?{border:"2px solid #f59e0b",background:"#fff7ed"}:{}),minHeight:80}}
                  onClick={()=>!isOff(ds)&&setDayModal({date:ds,reqs:all})}>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{...S.dayNum,...dayColor(ds)}}>{dn}</span>
                    {isHoliday(ds)&&<span style={S.holTag}>{JP_HOLIDAYS[ds]?.slice(0,4)}</span>}
                  </div>
                  {appr.slice(0,3).map(r=>{const emp=employees.find(a=>a.id===r.emp_id);
                    return <div key={r.id} style={{...S.chip,background:r.type==="有給休暇"?"#dbeafe":"#d1fae5",color:"#1e40af"}}>
                      {emp?.avatar} {(lang==="ja"?emp?.name:emp?.name_ko)?.split(" ")[0]}{r.half?t("半","반"):""}
                    </div>;
                  })}
                  {pend.length>0&&<div style={{...S.chip,background:"#fef3c7",color:"#92400e"}}>⏳{pend.length}</div>}
                  {appr.length>MAX_OFF&&<div style={{...S.chip,background:"#fee2e2",color:"#991b1b"}}>⚠超過</div>}
                </div>
              );
            }}
          />
          <Legend items={[{bg:"#d1fae5",tc:"#065f46",label:t("選択休暇","선택휴무")},{bg:"#dbeafe",tc:"#1e40af",label:t("有給休暇","유급휴가")},
            {bg:"#fef3c7",tc:"#92400e",label:t("⏳ 審査中","⏳ 심사중")},{bg:"#fff7ed",tc:"#92400e",label:t("⚠ 人員不足","⚠ 인원부족")}]}/>
        </>)}

        {tab==="pending"&&(<>
          <h3 style={S.secTitle}>{t("承認待ちリスト","승인 대기 목록")}</h3>
          {pendingList.length===0?<div style={S.empty}>{t("承認待ちはありません","대기 없음")}</div>
          :pendingList.map(r=>{const emp=employees.find(a=>a.id===r.emp_id);
            return (<div key={r.id} style={S.pendItem}>
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                <div style={S.smAv}>{emp?.avatar}</div>
                <div>
                  <div style={{fontSize:14,fontWeight:700}}>{lang==="ja"?emp?.name:emp?.name_ko}</div>
                  <div style={{fontSize:12,color:"#6b7280",marginTop:2}}>{r.type}{r.half?t("（半休）","（반차）"):""} · {r.date}</div>
                  {r.note&&<div style={{fontSize:11,color:"#9ca3af",marginTop:2}}>{r.note}</div>}
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
        ...
        {tab==="stats"&&(<>
          ...
          <div style={S.statsGrid}>
            {empOnly.map(emp=>{
              ...
              const sel=er.filter(r=>r.type==="選択休暇").length;
              const pct=Math.round((paid/20)*100);
              return (<div key={emp.id} style={S.statItem}>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                  <div style={S.smAv}>{emp.avatar}</div>
                  <div>
                    <div style={{fontSize:13,fontWeight:700}}>{lang==="ja"?emp.name:emp.name_ko}</div>
                  ...
                  {emp.must_change_password&&<span style={{...S.rolePill,background:"#fef3c7",color:"#92400e",fontSize:10,marginLeft:"auto"}}>仮PW</span>}
                </div>
                <div style={S.sRow}><span>{t("有給残","잔여유급")}</span>
                  <span style={{fontWeight:700,color:emp.remaining_paid_leave<5?"#ef4444":"#10b981"}}>{emp.remaining_paid_leave}{t("日","일")}</span></div>
                <div style={S.sRow}><span>{t("有給取得","유급사용")}</span><span>{paid}{t("日","일")}</span></div>
                <div style={S.sRow}><span>{t("選択休暇","선택휴무")}</span><span>{sel}{t("回","회")}</span></div>
                <div style={{marginTop:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#9ca3af",marginBottom:3}}>
                    <span>{t("消化率","소진율")}</span><span>{pct}%</span></div>
                  <div style={S.bar}><div style={{...S.barFill,width:`${pct}%`,background:pct>80?"#ef4444":"#6366f1"}}/></div>
                </div>
              </div>);
            })}
          </div>
      ...
      {dayModal&&(
        <div style={S.overlay} onClick={()=>setDayModal(null)}>
          <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
            <h3 style={S.modalTitle}>📋 {dayModal.date}</h3>
            {dayModal.reqs.length===0?<div style={S.empty}>{t("申請なし","신청 없음")}</div>
            :dayModal.reqs.map(r=>{const emp=employees.find(a=>a.id===r.emp_id);
              return (<div key={r.id} style={{...S.pendItem,marginBottom:8}}>
                <div style={{display:"flex",gap:10,alignItems:"center",flex:1}}>
                  <div style={S.smAv}>{emp?.avatar}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700}}>{lang==="ja"?emp?.name:emp?.name_ko}</div>
                    <div style={{fontSize:12,color:"#6b7280"}}>{r.type}{r.half?"(半)":""}</div>
                  </div>
                  <span style={{...S.pill,background:SC[r.status]}}>{SL[r.status]}</span>
                </div>
...
function MembersTab({lang,t,employees,addEmployee,updateEmployee,deleteEmployee,resetPassword}){
  ...
  return (
    <div>
      ...
      {employees.filter(e=>e.must_change_password).length>0&&(
        <div style={{...S.warnBanner,marginBottom:14}}>
          ⚠ {t(`${employees.filter(e=>e.must_change_password).length}名が仮パスワード未変更`,
             `${employees.filter(e=>e.must_change_password).length}명 임시PW 미변경`)}
        </div>
      )}
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {employees.length===0&&<div style={S.empty}>{t("社員がいません","등록된 직원이 없습니다")}</div>}
            ...
            <div style={{display:"flex",gap:12,alignItems:"center",flex:1}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontWeight:700,fontSize:14}}>{lang==="ja"?emp.name:emp.name_ko}</span>
                  {emp.must_change_password
                    ?<span style={{...S.rolePill,background:"#fef3c7",color:"#92400e",fontSize:10}}>{t("仮PW未変更","임시PW 미변경")}</span>
                    :<span style={{...S.rolePill,background:"#d1fae5",color:"#065f46",fontSize:10}}>{t("PW変更済","PW 변경완료")}</span>}
                </div>
                <div style={{fontSize:12,color:"#6b7280",marginTop:2}}>
                  {emp.dept} · {emp.selected_day_off}曜日 · {t("有給残","잔여")}: {emp.remaining_paid_leave}{t("日","일")}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginTop:4,flexWrap:"wrap"}}>
                  <span style={{fontSize:11,color:"#9ca3af"}}>ID: <code style={S.code}>{emp.login_id}</code></span>
                  <span style={{fontSize:11,color:"#9ca3af",display:"flex",alignItems:"center",gap:4}}>
        ...
        <div style={S.overlay} onClick={()=>setShowForm(false)}>
          <div style={{...S.modalBox,maxWidth:520}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <Field label={t("選択休暇曜日","선택휴무 요일")}>
                <select style={S.input} value={form.selectedDayOff} onChange={e=>setForm(f=>({...f,selectedDayOff:e.target.value}))}>
                  {["月","水","土"].map(d=><option key={d} value={d}>{d}曜日</option>)}
                </select>
              </Field>
              <Field label={t("有給残日数","잔여 유급일수")}>
                <input style={S.input} type="number" min="0" max="40" value={form.remainingPaidLeave}
            ...
            {(!form.name||!form.login_id||!form.tempPassword)&&(
              <div style={{fontSize:12,color:"#ef4444",marginTop:8}}>{t("必須項目を入力してください","필수 항목을 입력하세요")}</div>
            )}
            <div style={{display:"flex",gap:8,marginTop:20}}>
              <button style={S.primaryBtn} onClick={handleSave}>{editId!==null?t("更新","업데이트"):t("追加","추가")}</button>
              <button style={S.closeBtn} onClick={()=>setShowForm(false)}>{t("キャンセル","취소")}</button>
  ...
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
        ...
        <div style={S.fg}><label style={S.fl}>{t("現在のパスワード","현재 비밀번호")}</label>
          <div style={{position:"relative"}}>
            <input style={{...S.input,paddingRight:40}} value={cur} type={show?"text":"password"} onChange={e=>{setCur(e.target.value);setErr("");}}/>
            <button style={S.eyeBtn} onClick={()=>setShow(v=>!v)}>{show?"🙈":"👁"}</button>
          </div>
        </div>
        <div style={S.fg}><label style={S.fl}>{t("新しいパスワード","새 비밀번호")}</label>
          <input style={S.input} value={nw} type="password" onChange={e=>{setNw(e.target.value);setErr("");}} placeholder={t("6文字以上","6자 이상")}/>
        </div>
        <div style={S.fg}><label style={S.fl}>{t("パスワード確認","비밀번호 확인")}</label>
          <input style={S.input} value={nw2} type="password" onChange={e=>{setNw2(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&handleSave()}/>
        </div>
        {err&&<div style={{color:"#ef4444",fontSize:12,marginBottom:8}}>⚠ {err}</div>}
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button style={S.primaryBtn} onClick={handleSave}>{t("変更する","변경하기")}</button>
...
// REQUEST MODAL
// ══════════════════════════════════════════════════════════════════
function ReqModal({t,initDate,empId,onSubmit,onClose}){
  const [type,setType]=useState("選択休暇");
  const [date,setDate]=useState(initDate||"");
  const [note,setNote]=useState("");
  const [half,setHalf]=useState(false);
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
        <h3 style={S.modalTitle}>📋 {t("休暇申請","휴가 신청")}</h3>
        <div style={S.fg}><label style={S.fl}>{t("休暇種別","휴가 종류")}</label>
          {["選択休暇","有給休暇"].map(o=>(
            <label key={o} style={{display:"flex",alignItems:"center",gap:8,fontSize:14,cursor:"pointer",marginBottom:6}}>
              <input type="radio" checked={type===o} onChange={()=>setType(o)}/>
              {o==="選択休暇"?t("選択休暇（月・水・土）","선택휴무 (월·수·토)"):t("有給休暇","유급휴가")}
            </label>
          ))}
        </div>
        <div style={S.fg}><label style={S.fl}>{t("日付","날짜")}</label>
          <input type="date" style={S.input} value={date} onChange={e=>setDate(e.target.value)}/></div>
        {type==="有給休暇"&&(
          <div style={S.fg}><label style={{display:"flex",alignItems:"center",gap:8,fontSize:14,cursor:"pointer"}}>
            <input type="checkbox" checked={half} onChange={e=>setHalf(e.target.checked)}/>
            {t("半休（午前 or 午後）","반차 (오전 or 오후)")}
          </label></div>
        )}
        <div style={S.fg}><label style={S.fl}>{t("備考","비고")}</label>
          <textarea style={{...S.input,height:60,resize:"vertical"}} value={note}
            onChange={e=>setNote(e.target.value)} placeholder={t("理由を入力","사유 입력")}/></div>
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button style={S.primaryBtn} onClick={()=>onSubmit({empId,type,date,note,half})}>{t("申請する","신청하기")}</button>
          <button style={S.closeBtn} onClick={onClose}>{t("キャンセル","취소")}</button>
        </div>
      </div>
    </div>
...
function CalGrid({year,month,daysInMonth,firstDay,dayLabels,renderDay}){
  ...
  for(let d=1;d<=daysInMonth;d++) cells.push(renderDay(fmt(year,month,d),d));
  return (<div style={S.calGrid}>
    {dayLabels.map((l,i)=><div key={l} style={{...S.dayHdr,...(i===0?{color:"#ef4444"}:i===6?{color:"#3b82f6"}:{})}}>{l}</div>)}
    {cells}
  </div>);
}
function Legend({items}){
  return (<div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:12}}>
    {items.map(({bg,tc,label})=>(
      <div key={label} style={{display:"flex",alignItems:"center",gap:4,fontSize:11}}>
        <div style={{width:12,height:12,borderRadius:3,background:bg}}/><span style={{color:tc}}>{label}</span>
      </div>
    ))}
  </div>);
}
...
function Toast({msg,type}){
  return <div style={{...S.toast,background:type==="error"?"#ef4444":type==="warn"?"#f59e0b":"#10b981"}}>{msg}</div>;
}
function Field({label,req,children}){
  return (<div><label style={{display:"block",fontSize:12,fontWeight:700,color:"#374151",marginBottom:5}}>
    {label}{req&&<span style={{color:"#ef4444",marginLeft:3}}>*</span>}
...
const S={
  ...
  loginBg:{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",
    background:"linear-gradient(135deg,#6366f1 0%,#8b5cf6 50%,#a78bfa 100%)",padding:16},
  loginCard:{background:"#fff",borderRadius:20,padding:"32px 28px",width:"100%",maxWidth:400,boxShadow:"0 24px 80px rgba(0,0,0,0.2)"},
  loginTitle:{fontSize:22,fontWeight:900,textAlign:"center"},
  loginSub:{fontSize:12,color:"#9ca3af",textAlign:"center",marginBottom:8},
  eyeBtn:{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:16,color:"#9ca3af"},
  header:{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap",position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"},
  logo:{display:"flex",alignItems:"center",gap:10},
  logoIcon:{width:40,height:40,background:"#fff",border:"2px solid #e2e8f0",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center"},
  logoSub:{fontSize:11,color:"#94a3b8"},
  userChip:{display:"flex",alignItems:"center",gap:8,background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"6px 12px"},
  userAv:{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#a78bfa)",color:"#fff",fontSize:13,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"},
  rolePill:{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:6},
  logoutBtn:{background:"#f1f5f9",border:"none",borderRadius:8,padding:"6px 14px",fontSize:13,cursor:"pointer",fontWeight:600,color:"#374151"},
  langSwitch:{display:"flex",background:"#f1f5f9",borderRadius:8,overflow:"hidden"},
  langBtn:{padding:"5px 12px",border:"none",background:"transparent",cursor:"pointer",fontSize:12,color:"#64748b",fontWeight:600},
  langOn:{background:"#6366f1",color:"#fff",borderRadius:8},
  main:{maxWidth:1200,margin:"0 auto",padding:"20px 16px"},
  empLayout:{display:"grid",gridTemplateColumns:"260px 1fr",gap:20},
  sidebar:{display:"flex",flexDirection:"column",gap:14},
  card:{background:"#fff",borderRadius:14,padding:18,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"},
  bigAv:{width:52,height:52,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#a78bfa)",color:"#fff",fontSize:22,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 10px"},
  smAv:{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#a78bfa)",color:"#fff",fontSize:15,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},
  empName:{fontSize:16,fontWeight:700,textAlign:"center",marginBottom:4},
  empDept:{fontSize:12,color:"#6b7280",textAlign:"center",marginBottom:10},
  infoRow:{display:"flex",justifyContent:"space-between",alignItems:"center"},
  badge:{background:"#ede9fe",color:"#6d28d9",padding:"2px 8px",borderRadius:10,fontSize:12,fontWeight:700},
  ...
  histItem:{background:"#f8fafc",borderRadius:8,padding:10,marginBottom:8,fontSize:12},
  pill:{color:"#fff",fontSize:10,padding:"1px 7px",borderRadius:8,fontWeight:600},
  cancelBtn:{marginTop:6,background:"#fee2e2",color:"#991b1b",border:"none",borderRadius:6,padding:"3px 10px",fontSize:11,cursor:"pointer",fontWeight:600},
  calWrap:{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"},
  calGrid:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3},
  dayHdr:{textAlign:"center",fontSize:12,fontWeight:700,color:"#6b7280",padding:"6px 0"},
  dayCell:{border:"1px solid #f1f5f9",borderRadius:8,padding:"6px 7px",minHeight:60,cursor:"pointer",fontSize:12},
  dayCellOff:{background:"#f8fafc",cursor:"default"},
  dayNum:{fontSize:13,fontWeight:700,color:"#374151"},
  holTag:{fontSize:9,background:"#fee2e2",color:"#991b1b",borderRadius:4,padding:"1px 3px",maxWidth:50,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"},
  chip:{fontSize:10,padding:"1px 5px",borderRadius:5,fontWeight:700,marginTop:3,display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"},
  adminWrap:{background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"},
  tabBar:{display:"flex",gap:2,background:"#f8fafc",padding:"10px 14px 0",borderBottom:"1px solid #e2e8f0",alignItems:"center",overflowX:"auto"},
  tab:{padding:"8px 14px",border:"none",background:"transparent",cursor:"pointer",fontSize:13,fontWeight:600,color:"#6b7280",borderBottom:"2px solid transparent",marginBottom:-1,whiteSpace:"nowrap"},
  tabOn:{color:"#6366f1",borderBottom:"2px solid #6366f1"},
  csvBtn:{background:"#f1f5f9",border:"none",borderRadius:8,padding:"6px 14px",fontSize:12,cursor:"pointer",fontWeight:600,color:"#374151",marginBottom:2,whiteSpace:"nowrap"},
  select:{padding:"6px 10px",border:"1px solid #e2e8f0",borderRadius:8,fontSize:13,background:"#fff",cursor:"pointer"},
  warnBanner:{background:"#fef3c7",border:"1px solid #f59e0b",borderRadius:10,padding:"10px 16px",marginBottom:12,fontSize:13,color:"#92400e",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"},
  warnDate:{background:"#f59e0b",color:"#fff",borderRadius:6,padding:"1px 7px",fontSize:11,fontWeight:700},
  navBtn:{background:"#f1f5f9",border:"none",borderRadius:8,width:34,height:34,fontSize:20,cursor:"pointer",color:"#374151",fontWeight:700},
  pendItem:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",border:"1px solid #e2e8f0",borderRadius:10,marginBottom:10,background:"#fafafa",flexWrap:"wrap",gap:8},
  approveBtn:{background:"#d1fae5",color:"#065f46",border:"none",borderRadius:8,padding:"6px 14px",fontSize:13,fontWeight:700,cursor:"pointer"},
  rejectBtn:{background:"#fee2e2",color:"#991b1b",border:"none",borderRadius:8,padding:"6px 14px",fontSize:13,fontWeight:700,cursor:"pointer"},
  memberRow:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",border:"1px solid #e2e8f0",borderRadius:12,background:"#fafafa",flexWrap:"wrap",gap:8},
  editBtn:{background:"#e0f2fe",color:"#0369a1",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"},
  delBtn:{background:"#fee2e2",color:"#991b1b",border:"none",borderRadius:8,padding:"6px 10px",fontSize:14,cursor:"pointer"},
  secTitle:{fontSize:15,fontWeight:800,marginBottom:14,color:"#1e293b"},
  statsGrid:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12},
  statItem:{background:"#f8fafc",borderRadius:12,padding:14,border:"1px solid #e2e8f0"},
  sRow:{display:"flex",justifyContent:"space-between",fontSize:13,color:"#374151",marginTop:6},
  ...
  modalBox:{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:440,boxShadow:"0 20px 60px rgba(0,0,0,0.2)",maxHeight:"90vh",overflowY:"auto"},
  modalTitle:{fontSize:18,fontWeight:800,marginBottom:20,color:"#1e293b"},
  fg:{marginBottom:14},
  fl:{display:"block",fontSize:12,fontWeight:700,color:"#374151",marginBottom:6},
  input:{width:"100%",padding:"9px 12px",border:"1px solid #e2e8f0",borderRadius:8,fontSize:14,outline:"none",background:"#f8fafc",boxSizing:"border-box"},
  primaryBtn:{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",border:"none",borderRadius:10,padding:"10px 24px",fontSize:14,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(99,102,241,0.3)"},
  closeBtn:{background:"#f1f5f9",color:"#374151",border:"none",borderRadius:10,padding:"10px 20px",fontSize:14,fontWeight:600,cursor:"pointer"},
  toast:{position:"fixed",top:20,right:20,zIndex:9999,color:"#fff",padding:"12px 20px",borderRadius:12,fontSize:13,fontWeight:600,boxShadow:"0 4px 20px rgba(0,0,0,0.2)",maxWidth:360},
  code:{fontSize:11,background:"#e2e8f0",padding:"1px 6px",borderRadius:4,fontFamily:"monospace"},
  empty:{color:"#9ca3af",textAlign:"center",padding:"20px 0",fontSize:13},
};
